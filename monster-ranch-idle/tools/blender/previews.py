...
three-quarter view, looping) and preview_<clip>_sheet.png (six side-view frames).
"""
...
for clip, info in manifest["clips"].items():
    ms = int(1000 * info["step"] / manifest["fps"])
    for view, files in info["views"].items():
        ...
        if view == "front34":
            ...
            picks = [shots[int(i * len(shots) / cols)] for i in range(cols)]
            sheet = Image.new("RGB", (picks[0].width * cols, picks[0].height))
            for i, s in enumerate(picks):
                sheet.paste(s, (i * s.width, 0))
            sheet.save(OUT / f"preview_{clip}_sheet.png")
